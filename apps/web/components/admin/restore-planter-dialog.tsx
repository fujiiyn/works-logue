...
interface Props {
  ...
  onRestored: (updated: AdminPlanterItem) => void;
}

export function RestorePlanterDialog({
  planter,
  open,
  onOpenChange,
  onRestored,
  ...
  async function handleConfirm() {
    ...
    setError(null);
    try {
      const updated = await adminApi.restorePlanter(planter.id);
      onRestored(updated);
      onOpenChange(false);
    } catch (e) {
  ...
  return (
    <Dialog
      ...
      title="この Planter を復元しますか？"
      description="復元後の状態は Seed になります。"
      footer={
        <>
          <button
            type="button"
