...
export function PlanterCard({ planter }: PlanterCardProps) {
  ...
  return (
    <article
      className="relative border-b border-border py-[18px]"
      data-testid="planter-card"
    >
      {/* Background overlay link covers the whole card */}
      <Link
        href={`/p/${planter.id}`}
        className="absolute inset-0 z-0"
        aria-label={planter.title}
      />

      {/* Foreground content with inner links above the overlay */}
