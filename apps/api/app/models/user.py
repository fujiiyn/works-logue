...
class User(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    headline: Mapped[str | None] = mapped_column(String(60), nullable=True)
    cover_url: Mapped[str | None] = mapped_column(nullable=True)
    location: Mapped[str | None] = mapped_column(String(100), nullable=True)
    x_url: Mapped[str | None] = mapped_column(nullable=True)
    linkedin_url: Mapped[str | None] = mapped_column(nullable=True)
    wantedly_url: Mapped[str | None] = mapped_column(nullable=True)
    website_url: Mapped[str | None] = mapped_column(nullable=True)
    pending_avatar_path: Mapped[str | None] = mapped_column(nullable=True)
    pending_cover_path: Mapped[str | None] = mapped_column(nullable=True)
